<template>
  <div class="h-100">
    ...
    <div class="mt-3 service-container2">
      <div class="d-flex h-100">
        <div class="left d-flex flex-column mr-3 h-100">
          <el-card class="h-100">
            <div class="d-flex justify-content-between align-items-center">
              ...
              <el-button type="primary" @click="handleAdd()" v-if="worksheetInfo.status != 3"><i class="el-icon-plus el-icon--left"></i>问题</el-button>
            </div>
            <data-tables-server :data="service_list" layout="table" :table-props="tableProps">
              ...
              <el-table-column prop="is_resolve" label="是否解决">
                <template slot-scope="scope">
                  <span v-if="scope.row.is_resolve == 1"><i class="dot bg-warning mr-1"></i>否</span>
                  <span v-else-if="scope.row.is_resolve == 2"><i class="dot bg-success mr-1"></i>是</span>
                </template>
              </el-table-column>
        ...
        <el-card class="right bg-white h-100">
          ...
          <div class="mt-3 worksheetInfo">
            <div class="mb-3 text-truncate"><span class="opacity-60 mr-2">工单编号：</span>{{worksheetInfo.question_number}}</div>
            <div class="mb-3 text-truncate"><span class="opacity-60 mr-2">标题：</span>{{worksheetInfo.title}}</div>
						<div class="mb-3 text-truncate"><span class="opacity-60 mr-2">问题描述：</span>{{worksheetInfo.question_content}}</div>
						<div class="mb-3 text-truncate"><span class="opacity-60 mr-2">优先级：</span>{{worksheetInfo.level}}</div>
						<div class="mb-3 text-truncate"><span class="opacity-60 mr-2">工单状态：</span>
              ...
              <span v-else-if="worksheetInfo.status == 3"><i class="dot bg-success mr-1"></i>已完成</span>
						...
						<div class="mb-3 text-truncate"><span class="opacity-60 mr-2">提交时间：</span>{{worksheetInfo.createtime}}</div>
            ...
            <div class="mb-3 text-truncate"><span class="opacity-60 mr-2">校方备注：</span>{{worksheetInfo.school_remark}}</div>
            ...
            <div class="mb-3 text-truncate"><span class="opacity-60 mr-2">企业备注：</span>{{worksheetInfo.company_remark}}</div>
      ...
      <serviceDetail-add :serviceData="serviceData"></serviceDetail-add>
    </div>
  </div>
</template>
...
<script>
  import GlobalTips from "@/components/GlobalTips";
	...
	import ServiceDetailAdd from "./ServiceDetailAdd";
