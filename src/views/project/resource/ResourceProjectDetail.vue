			...
			<el-col :span="24">
				<el-card class="h-100">
					...
					<div class="d-flex align-items-center flex-wrap">
						...
						<p class="w-100 mb-3"><span class="opacity-60 mr-2">审核状态：</span>
							<span v-if="basic.status == 1"><i class="dot bg-primary mr-1"></i>待确认</span>
							<span v-else-if="basic.status == 2"><i class="dot bg-warning mr-1"></i>待审核</span>
							<span v-else-if="basic.status == 3"><i class="dot bg-success mr-1"></i>审核成功</span>
							<span v-else-if="basic.status == 4"><i class="dot bg-danger mr-1"></i>审核失败</span>
							<span v-else-if="basic.status == 5"><i class="dot bg-cyan mr-1"></i>已回收</span>
						</p>
						<p class="w-100 mb-3"><span class="opacity-60 mr-2">申请时间：</span>{{basic.applytime}}</p>
						<p class="w-100 mb-3"><span class="opacity-60 mr-2">审核时间：</span>{{basic.checktime}}</p>
						<p class="w-100 mb-0" v-if="basic.type == 2"><span class="opacity-60 mr-2">资源申请备注：</span>{{ basic.apply_remark}}</p>
						<p class="w-100 mb-0" v-else><span class="opacity-60 mr-2">备注：</span>{{ basic.remark }}</p>
					</div>
				</el-card>
			</el-col>
		</el-row>
...
<script>
	...
	export default {
		...
		methods:{
			// 获取数据
			initData(){
				this.$api.project_apply_resourceDetail({
