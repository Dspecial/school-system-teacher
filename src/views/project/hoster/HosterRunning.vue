  ...
  <div class="page_detail">
    ...
    <el-card class="mt-3 bg-white">
      ...
      <el-form :model="projectForm" :rules="rules" ref="projectForm" label-width="110px" label-position="top" class="pl-3 pr-3">
        ...
        <!-- 实施流程的额外参数 -->
        <el-row :gutter="20">
          <div v-for="(formItem, j) in projectForm.runningExtraForms" :key="j">
            <!-- 字段类型:1=文本框,2=数字框,3=下拉单选,4=日期选择,5=文件上传(单选),6=文本域,7=富文本,
						8=时间选择,9=下拉多选,10=复选,11=单选,12=数组,13=图片上传(单选),14=图片上传(多选),15=文件上传(多选) -->
            <!-- 1=文本框 -->
            <el-col :span="12" v-if="formItem.name_type == 1">
        ...
        <el-row :gutter="20">
          <el-col :span="24">
            <el-form-item label="合同规定付款计划" class="payment_item" required>
              <div slot="label" class="d-flex justify-content-between">
                <span>合同规定付款计划</span>
                <span class="text-primary cursor-pointer" @click="addPayinfo(projectForm.agree_payinfo)">
                  <i class="el-icon-plus mr-1"></i>
                  付款信息
                </span>
              </div>
...
export default {
  ...
  methods: {
      ...
      this.$api
        ...
        .then(data => {
          if (data.code == 0) {
            ...
            if (!data.data.runningextra) {
              this.getNodeExtra();
            } else {
              var runningExtraForms = data.data.runningextra;
              ...
              this.projectForm.runningExtraForms = runningExtraForms;
            }
          } else {
            this.$message.error(data.msg);
          }
    ...
    submitForm(formName) {
      ...
      var isExtraArr = this.commonJs.isEmpty(this.projectForm.runningExtraForms);
      if (!isExtraArr) {
        sendExtra = this.projectForm.runningExtraForms.map(item => {
          if (item.name_type == 5 || item.name_type == 13 || item.name_type == 14 || item.name_type == 15) {
            var nameArray = item.value.map(file => {
              if (this.commonJs.isEmpty(file.response)) {
                return file.path;
