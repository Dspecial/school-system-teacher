    ...
    <el-card class="mt-3 bg-white">
      ...
      <el-form :model="projectInfo" ref="projectInfo" label-width="110px" label-position="left" class="pl-3 pr-3">
        <el-row :gutter="20">
          ...
          <el-col :span="8">
            <el-form-item label="年份">{{ projectInfo.projecttime }} 年</el-form-item>
          </el-col>
          <el-col :span="8">
      ...
      <el-form ref="checkform" :model="checkform" class="pl-3 pr-3" label-position="top" label-width="110px" :rules="rules">
        <el-form-item label="绑定资源" class="payment_item" v-if="check_info.node_id == 14">
          <div slot="label" class="d-flex justify-content-between">
            ...
            <span>绑定资源</span>
            <span class="text-primary cursor-pointer" @click="addPro(checkform.bindResources)">
                ...
                <div style="width:95%">
                  <el-row align="middle" :gutter="20">
                    ...
                    <el-col :span="6" v-if="cell.type == '2'" class="mb-2">
                      <el-date-picker
                        v-model="cell.usetime"
                        type="date"
                        clearable
                        placeholder="选择具体到期时间"
                          ...
                          <div v-for="(field, index) in cell.detailjson" :key="index">
                            ...
                            <el-col :span="6" v-if="field.name_type == 2">
                              <el-form-item :label="field.title" :required="field.is_required == 2">
                                <el-input
                                  ...
                                  :placeholder="field.placeholder == '' ? '请输入' + field.title : field.placeholder"
                                ></el-input>
                              </el-form-item>
                            </el-col>
                            ...
                            <el-col :span="6" v-if="field.name_type == 3">
                              <el-form-item :label="field.title" :required="field.is_required == 2">
                                <el-select
                                  v-model="field.val"
                                  :placeholder="field.placeholder == '' ? '请选择' + field.title : field.placeholder"
                                  class="w-100"
                                  clearable
                                >
                            ...
                            <el-col :span="24" v-if="field.name_type == 5">
                              <el-form-item :label="field.title" :required="field.is_required == 2">
                                <el-upload action="#" drag :auto-upload="false" :on-success="handleAvatarSuccess" :before-upload="beforeAvatarUpload">
                                  <i class="el-icon-upload"></i>
                                  <div class="el-upload__text">
                                    将{{ field.title }}文件拖到此处，或
                                    <em>点击上传</em>
                                  </div>
                                </el-upload>
                              </el-form-item>
        ...
        <el-form-item label="承办人" prop="hoster_ids" v-if="need_set_charge == 1 && checkform.check_state == 2">
          <el-select
            ...
            @clear="selectClear"
          >
            <el-option v-for="item in uidOptions" :key="item.id.toString()" :label="item.name" :value="item.id.toString()">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        hoster_ids: [{ required: true, message: '请选择承办人', trigger: 'change' }],
  ...
  methods: {
    ...
    // 获取资源分类
    initCate() {
      this.$api.getResource_cate({}).then(data => {
        if (data.code == 0) {
          // 回调成功的方法
    ...
    // 获取供应商
    initSupplierOptions() {
      this.$api.getResource_supplier({}).then(data => {
        if (data.code == 0) {
