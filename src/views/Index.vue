          ...
          <div class="tab_content">
            <el-row class="isCell handleType">
              <template v-for="(type, index) in handleType">
                <el-col :span="8" class="text-center cursor-pointer" :key="index" @click.native="goTypeRoutine(index)">
          ...
          <div class="d-flex justify-content-between align-items-center isCell">
            ...
            <div class="tab_nav">
              <template v-for="(year, index) in handleYear">
                <span :key="index" :class="['cursor-pointer ml-3', handleYearIndex == year.id ? 'active' : '']" @click="handleYearTab(year.id)">
                  {{ year.title }}
                </span>
              </template>
            </div>
...
export default {
  ...
  methods: {
    // 获取我的项目
    initState() {
      this.$api.dashboard_state({}).then(data => {
        if (data.code == 0) {
