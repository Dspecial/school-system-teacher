...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        dept_id: [{ required: false, message: '请选择负责人所在部门', trigger: 'change' }],
        leader_id: [{ required: false, message: '请选择负责人', trigger: 'change' }],
        budget_amount: [
  ...
  methods: {
    // 获取项目分类
    initCate() {
      this.$api.getProjectCate({}).then(data => {
        if (data.code == 0) {
          // 回调成功的方法
    ...
    // 获取负责人所在部门列表
    initDept() {
      this.$api.getDeptOptions({}).then(data => {
        if (data.code == 0) {
          // 回调成功的方法
